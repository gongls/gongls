...
db = {
    ...
    find:function(collection,query_obj,select_obj,callback){
        MongoClient.connect(mongoUrl, function (err, db) {
            assert.equal(null, err);
            db.collection(collection).find(query_obj).sort(select_obj.sort).limit(select_obj.limit).skip(select_obj.skip).toArray(function (err, result) {
                assert.equal(err, null);
                callback(result);
                db.close();
            });
